...
them) or only when a profiler is attached; here we drive the very same module functions directly and
via a ThreadExecutor, deterministically and on every platform (no subprocess-coverage machinery)."""
...
class FakeProfiler:
    """A deterministic WorkerProfiler stand-in (no pyinstrument timing): flush always yields bytes."""
...
def test_thread_profiling_runs_profiler_in_process() -> None:
    """ThreadExecutor + a profiler factory runs _thread_profiler + the _run_with_emit flush block in
    the main process (in a process pool they would run in a child)."""
    mon = ProfMonitor()
